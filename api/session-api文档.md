...
获取当前 API Key 可见的所有项目及其会话列表。轻量级接口，只返回会话元数据（不包含消息内容）。

```
GET /api/projects
```

无参数。使用 `x-api-key` 头部认证（同其他 API）。
    ...
    "sessions": [
      {
        ...
        "lastActivity": "2024-07-01T12:30:00Z",
        "messageCount": 10,
        "cwd": "/working/dir",
        "firstPrompt": "第一条提示词"
      }
...
|------|------|------|
| `name` | string | 项目唯一标识名 |
| `displayName` | string | 项目显示名 |
| `fullPath` | string | 项目完整路径 |
| `path` | string | 项目路径 |
| `lastActivity` | number | 最后活动时间戳 (ms) |
| `sessions` | array | 会话列表（见下） |
| `sessionMeta.total` | number | 总会话数 |
| `sessionMeta.hasMore` | bool | 是否有更多 |
| `taskmaster.hasTaskmaster` | bool | 是否启用 Taskmaster |
| `alwaysOn.enabled` | bool | 是否启用 AlwaysOn |
...
| `cwd` | string | 工作目录 |
| `firstPrompt` | string | 第一条提示词 |

### 判断 Session 是否存在

- 从返回的 `sessions` 数组中查找目标 `id` → 找到则 session 存在
- `messageCount > 0` → session 有消息内容

### 注意事项

- 此接口 **不包含实时处理状态**（`isProcessing`）。要判断是否正在处理中，需使用 WebSocket（见第 6 节）。
- 返回所有 API Key 可见的项目，不支持按项目路径过滤。
